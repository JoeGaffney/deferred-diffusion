...
from local_worker.context import WorkerContext


def register_task(t_name: str):
    """Dynamically register task specifically for local_worker."""
    # Registering as `local_worker.task_name`
    full_task_name = f"local_worker.{t_name}"

    @celery_app.task(name=full_task_name, bind=True)
    def wrapper(self, params_dict: dict):
        mod = importlib.import_module(f"local_worker.tasks.{t_name}.execute")
