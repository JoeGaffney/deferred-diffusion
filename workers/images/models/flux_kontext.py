...
# NOTE there is a better pipeline for inpainting, but it is not yet available in the Flux-Kontext model.
def inpainting_call(context: ImageContext):
    pipe = AutoPipelineForInpainting.from_pipe(
        get_pipeline(context.get_pipeline_config()), requires_safety_checker=False
    )
